...
export default function DetallePedidoPage() {
  ...
  return (
    <div className="max-w-xl mx-auto p-6 bg-white rounded-lg shadow-md text-blue-900">
      <h2 className="text-2xl font-semibold mb-4 text-center text-blue-900">Pedido #{p.id}</h2>
      <ul className="space-y-2 text-blue-900">  
        <li>    <strong>Cliente:</strong> {p.cliente ? `${p.cliente.nombre} ${p.cliente.apellido}` : p.clienteId}  </li>
      ...
      <h3 className="mt-4 font-medium text-blue-900">Detalles de Prenda</h3>
        ...
        <tbody>
          {p.detalles.map(d=>(
            <tr key={d.id} className="border-t">
              <td className="p-2  text-blue-900">{d.modelo}</td>
              <td className="p-2 text-blue-900">{d.talla}</td>
              <td className="p-2 text-blue-900">{d.cantidad}</td>
            </tr>
          ))}
        </tbody>
      </table>
